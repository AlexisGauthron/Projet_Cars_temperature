...
Dernier modèle YOLO (2026) fine-tuned pour la détection de visages.
"""
...
class YOLOv12FaceDetector(BaseDetector):
    """YOLOv12 Face detector - Latest YOLO (2026)."""
    ...
    def __init__(self):
        ...
        try:
            ...
            model_path = MODELS_DIR / "yolov12" / "yolov12n-face.pt"
            if model_path.exists():
                self.model = YOLO(str(model_path))
                self._log_init_success()
            else:
                # Télécharger depuis HuggingFace si disponible
                try:
                    from huggingface_hub import hf_hub_download
                    model_file = hf_hub_download(
                        repo_id="foduucom/yolov12-face",
                        filename="yolov12n-face.pt"
                    )
